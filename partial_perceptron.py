...
class Partial_Perceptron:
        ...
        for pos, obs in enumerate(features):
            ...
            if output[pos] != self.o_id:
                for next_tag in range(self.num_labels):
                    ...
                    next_tag_expr = for_expr + self.transition[next_tag] + obs_broadcast if pos ==0 or output[pos-1] != self.o_id else for_expr + obs_broadcast
                    alphas_t.append(max_score(next_tag_expr))
                for_expr = dy.concatenate(alphas_t)
            # for_expr = dy.max_dim(alphas_t)
            # dy.emax()
        terminal_expr = for_expr + self.transition[self.label2idx[STOP]] if output[-1] != self.o_id else for_expr
    ...
    def forward_labeled(self, id, features, tags, is_prediction):
        ...
        for i, obs in enumerate(features):
            ...
            if not is_prediction[i+1]:
                score = score + dy.pick(self.transition[tags[i + 1]], tags[i]) + dy.pick(obs, tags[i + 1]) if not is_prediction[i] else score + dy.pick(obs, tags[i + 1])
    ...
    def negative_log(self, id, x, y, x_chars=None):
        ...
        unlabeled_score = self.forward_labeled(id, features, best_path, is_prediction)
