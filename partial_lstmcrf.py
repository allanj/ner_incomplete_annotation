import dynet as dy
from utils import log_sum_exp,check_bies_constraint
import numpy as np
...
class Partial_BiLSTM_CRF:
    ...
    def forward_labeled(self, id, features):
        init_alphas = [-1e10] * self.num_labels
        init_alphas[self.label2idx[START]] = 0
        for_expr = dy.inputVector(init_alphas)
        # print(id)
        ...
        tensor = dy.inputTensor(self.mask[id])
        for pos, obs in enumerate(features):
            ...
            for_expr = dy.concatenate(alphas_t) + tensor[pos]
        terminal_expr = for_expr + self.transition[self.label2idx[STOP]]
        alpha = log_sum_exp(terminal_expr, self.num_labels)
        return alpha
    ...
    def negative_log(self, id, x, y, x_chars=None):
        ...
        labeled_score = self.forward_labeled(id, features)
